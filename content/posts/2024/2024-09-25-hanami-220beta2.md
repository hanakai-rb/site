...
We also completed the last piece of ROM integration work: integrated [commands](https://hanakai.org/learn/rom/v5.0/core-concepts/commands/) and [mappers](https://hanakai.org/learn/rom/v5.0/core-concepts/mappers/). These are advanced components of ROM, and if you need them in a Hanami app, now you can place them in `app/db/commands/` and `app/db/mappers/` and they’ll be automatically registered.

## Full validation contracts for actions

By popular demand, we’ve taken a little detour from our database work to implement support for full [dry-validation](https://hanakai.org/learn/dry/dry-validation/) contracts for your actions. Now you can use `contract` within an action class and access all the features of dry-validation, especially rules:
