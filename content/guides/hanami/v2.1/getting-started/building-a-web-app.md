...
In Hanami, [providers](//guide/app/providers) offer a mechanism for configuring and using complex dependencies, like database connections, within your app.
...
To access the books relation within the view, we can use Hanami's "Deps mixin". Covered in detail in the [container and components](//guide/app/container-and-components) section of the Architecture guide, the Deps mixin gives each of your app's components easy access to the other components it depends on to achieve its work. We'll see this in more detail as these guides progress.
...
From here you might want to look in more detail at [routing](//guide/routing) and [actions](//guide/actions), or explore Hanami's [app architecture](//guide/app), starting with its [component management](//guide/app/container-and-components) and [dependency injection](//guide/app/container-and-components) systems. Or you may want to head straight to the front end and learn more about Hanami's [views](//guide/views) and [assets management](//guide/assets)
