...
The new format config API is documented in [Formats and media types](//guide/actions/formats-and-media-types). To use it, make the following changes wherever you configure your actions (in your app class using `config.actions`, or directly in your action classes using `config` or `format`).
