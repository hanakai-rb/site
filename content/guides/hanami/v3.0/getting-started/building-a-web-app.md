...
In this guide, we'll implement the index, show, new, create, edit, and update actions. We use the `only:` option to specify which routes to create, adding each action as we implement it.
...
- `GET /books/1/edit` → `books.edit` (to show the form)
- `PATCH /books/1` → `books.update` (to handle the form submission)
