...
In this example, `action` is an instance of `Bookshelf::Actions::Books::Index`. To make a request to the action, we can call it with an empty parameters hash. The return value is a serialized [Rack](https://github.com/rack/rack) response. In this test we’re asserting that the returned response is successful (status is in `2XX` range).
