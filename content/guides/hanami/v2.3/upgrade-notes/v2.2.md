...
- Update `spec/spec_helper.rb` and update any `require` or `require_relative` lines for files in `spec/support/` with the following:
  ...
  require "pathname"
  SPEC_ROOT = Pathname( **dir** ).realpath.freeze

  # <leave main file contents here>

  SPEC*ROOT.glob("support/\**/\_.rb").each { |f| require f }
...
- Create `spec/support/db/cleaning.rb`:
  ...
  RSpec.configure do |config| # Returns all the configured databases across the app and its slices. # # Used in the before/after hooks below to ensure each database is cleaned between examples. # # Modify this proc (or any code below) if you only need specific databases cleaned.
  all_databases = -> {
...
- Create `spec/support/operations.rb`:
  ...
  RSpec.configure do |config| # Provide `Success` and `Failure` for testing operation results
