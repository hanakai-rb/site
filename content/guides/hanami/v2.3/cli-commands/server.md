...
When the application code changes, the application is reloaded, so for new incoming HTTP requests, the app works with the most recent version of the code. It’s convenient feature to avoid developers to stop and start the web server for each code change.
